...
def recalcular_posicao(
    *,
    ...
    if tipo == "COMPRA":
        # Novo custo = custo atual + (qtd_compra * preco_unitario)
        custo_atual = quantidade_atual * preco_medio_atual
        custo_novo = custo_atual + (quantidade_operacao * preco_unitario)
        ...
        return PosicaoResult(
            ...
            custo_total=_round_money(custo_novo, 8),
        )
        ...
        # Na venda, o preço médio não muda, apenas reduz custo total.
        custo_novo = quantidade_nova * preco_medio_atual

        return PosicaoResult(
            quantidade=_round_money(quantidade_nova, 8),
            preco_medio=_round_money(preco_medio_atual, 8),
            custo_total=_round_money(custo_novo, 8),
        )
